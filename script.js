...
function loadAdminTable() {
  ...
  if (volunteers.length === 0) {
    tbody.innerHTML =
      `<tr><td colspan="11">No applications submitted</td></tr>`;
    return;
  }
  ...
  volunteers.forEach((v, i) => {
    tbody.innerHTML += `
      <tr>
        ...
        <td>
          <button onclick="updateStatus(${i},'Approved')" class="approve">Approve</button>
          <button onclick="updateStatus(${i},'Rejected')" class="reject">Reject</button>
        </td>
      </tr>
    `;
  });
